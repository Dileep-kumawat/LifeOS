...
- [Vercel Monorepo Root Directory & SPA Routing]: Created `web/vercel.json` with SPA route rewrites (`/(.*) -> /index.html`); documented Vercel project configuration requiring Root Directory set to `web` with external source inclusion enabled to avoid Vercel treating the monorepo `/api` Express routes as Serverless Functions (preventing the 12-function Hobby plan limit error).
- [Vercel Rollup Native Linux Binary Resolution (npm #4828)]: Added `@rollup/rollup-linux-x64-gnu` and `@rollup/rollup-linux-x64-musl` to `optionalDependencies` in `package.json` and `web/package.json`; regenerated `package-lock.json` lockfile entries; resolved Vercel deployment crash (`Cannot find module @rollup/rollup-linux-x64-gnu`) caused by npm lockfiles generated on Windows omitting platform-specific Rollup binaries during Linux CI/CD builds.
- [Production Monorepo TypeScript Build & Render Deploy Fix]: Excluded test files (`src/**/__tests__/**`, `src/**/*.test.ts`) from `packages/shared/tsconfig.json` and `api/tsconfig.json`; added `vitest` to `packages/shared/package.json` devDependencies; resolved Render production build failure (`TS2307: Cannot find module 'vitest'`) during `npm install` postinstall where devDependencies are skipped under `NODE_ENV=production`; verified clean compilation across `@lifeos/shared` and `@lifeos/api`.
- [AI Fallback Chain Model Upgrades (FR-2.10, FR-2.14)]: Upgraded default Groq model from unsupported `llama-3.3-70b-versatile` to `openai/gpt-oss-120b` and default Google Gemini model from deprecated `gemini-1.5-flash` to `gemini-3.6-flash` in `api/src/services/ai/providers.ts`; added `process.env.GROQ_MODEL` and `process.env.GEMINI_MODEL` environment variable overrides; resolved cascading 404 errors during primary provider (Mistral) 429 rate limit fallback.
...
- **Read-Through Caching & Invalidation Architecture**:
  - **Redis Fail-Open Rule (applies to ALL Redis call sites)**: Because `ioredis` runs with `maxRetriesPerRequest: null`, any `await` on Redis while disconnected queues offline forever and hangs the HTTP request (browser "Pending"). Every Redis touch MUST fail open: return immediately when `redis.status` is defined and not `"ready"` (treat `undefined` status as usable so unit-test mocks still run), and race every command against a strict timeout (rate limiter 1s, cache 1s, queue 2s), degrading todirect execution / `{ queued: false }` with `logger.warn` instead of hanging or throwing.
