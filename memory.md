...
- [Web Responsive Notifications Popover Viewport Clamping]: Resolved notification popover overflowing the left edge of responsive/mobile viewports; added viewport boundary collision detection with dynamic isomorphic layout effect measurement in web/src/components/ui/Popover.tsx, shifting overflowing popovers into the screen with a 12px safe margin; made PopoverContent width responsive (w-[calc(100vw-1.5rem)] sm:w-80 max-w-[calc(100vw-1.5rem)]) and preserved transform styles; added pointerdown event listener alongside mousedown for instant mobile touch dismissal; updated NotificationPanel.tsx list container to max-h-[calc(100dvh-8rem)] sm:max-h-96 to prevent vertical overflow on short mobile screens; verified typecheck clean, 35/35 web vitest tests passing, and Vite production bundle build succeeded cleanly.
- [Web Silent JWT Refresh & Rotated Token Persistence]: Resolved premature web logout bug caused by expired access tokens; implemented resilient 401 response interceptor with single-flight mutex (`refreshPromise`) in `web/src/lib/apiClient.ts`; added dual-channel refresh token persistence utility (`web/src/lib/tokenStorage.ts`) with in-memory fallback for test/SSR; updated `refreshAccessToken()` to send `storedRefreshToken` in `{ refreshToken }` body and `x-refresh-token` header alongside `withCredentials: true` cookies; persisted rotated refresh tokens on `/auth/login`, `/auth/register`, and `/auth/refresh` in `LoginPage.tsx`, `RegisterPage.tsx`, and `apiClient.ts`; updated `useAuthStore.clearAuth()`, `SettingsPage.tsx`, and `RootLayout.tsx` to revoke backend refresh tokens and clear storage on logout; added infinite loop protection against auth routes (`/auth/login`, `/auth/refresh`, `/auth/register`); verified with 10 comprehensive tests in `web/src/lib/__tests__/authFlow.test.ts` (30/30 web tests passing), 0 TypeScript errors across all 4 monorepo workspaces, and clean Vite production build.
- [Mobile-v2 Native Shell Context Detection & Download Page Concealment]: Implemented reliable dual-signal detection (window.Capacitor?.isNativePlatform() and injected window.LIFEOS_APP_SHELL = true) via centralized utility and hook web/src/lib/platform.ts (isInsideNativeApp, useIsInsideNativeApp, usePlatform); updated web/src/routes/RootLayout.tsx to hide all 3 APK download entry points (mobile top header, mobile drawer, and desktop sidebar) when inside native app shell; wrapped /download route in DownloadRouteGuard (web/src/App.tsx) and DownloadAppPage.tsx to redirect immediately to "/" with replace: true; injected window.LIFEOS_APP_SHELL = true in mobile-v2 MainActivity.java (WebViewClient onPageStarted and onPageFinished) and mobile-v2/src/main.ts; verified 100% desktop and mobile browser backward safety, 20/20 web vitest tests passing, 0 TypeScript errors, clean web production bundle build, and clean Android Java compilation.
- [Mobile-v2 & Web Full-Bleed Edge-to-Edge Safe Area Insets Handling]: Resolved mobile UI clipping on notch, status bar, and gesture navigation bars in Capacitor 7 Android client; kept full edge-to-edge across Android 11–15 with WindowCompat.setDecorFitsSystemWindows(false) and transparent system bars in MainActivity.java and capacitor.config.ts; updated web/index.html viewport to viewport-fit=cover so Chromium exposes CSS env(safe-area-inset-*); updated RootLayout.tsx with pt-[env(safe-area-inset-top,0px)] header, pb-[calc(0.375rem+env(safe-area-inset-bottom,0px))] bottom navbar, dynamic canvas pt/pb clearance, drawer safe-padding, and ChatPage.tsx header/input bar safe-area padding; verified 100% desktop backward safety, clean web production compilation, and clean standalone Android debug APK compilation.
...
- **Web JWT Silent Refresh & Token Rotation Convention**:
  - **Volatile Access Token**: Access token is held strictly in volatile Zustand store memory (`useAuthStore`) and never written to disk or localStorage.
  - **Dual-Channel Refresh Persistence**: Refresh tokens are stored via `tokenStorage` (`web/src/lib/tokenStorage.ts`) in `localStorage` with in-memory fallback, and sent on refresh via both `{ refreshToken }` body and `x-refresh-token` header, complemented by `withCredentials: true` httpOnly cookies. This guarantees compatibility across cross-origin deployments, privacy-hardened browsers, and Capacitor WebViews where cookies are blocked.
  - **Single-Flight Mutex**: In `apiClient.ts`, `refreshAccessToken()` deduplicates concurrent refresh calls using a shared in-flight promise (`refreshPromise`) so parallel TanStack queries that return 401 trigger exactly ONE `/auth/refresh` HTTP call.
  - **Rotated Token Persistence**: Every successful refresh issues a new rotated refresh token from the server. `web/src/lib/apiClient.ts` immediately updates `tokenStorage` with the new token. Storing the rotated token prevents single-use revocation failures and avoids triggering theft/reuse detection (which revokes all user sessions).
  - **Infinite Loop Guard**: Interceptor explicitly bypasses 401 handling for `/auth/login`, `/auth/refresh`, and `/auth/register`. Logout endpoints (`SettingsPage.tsx`, `RootLayout.tsx`) send `{ refreshToken }` to `/auth/logout` so the backend revokes the MongoDB document, then clears client state.
