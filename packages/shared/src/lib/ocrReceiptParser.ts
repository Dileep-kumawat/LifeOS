 ...
 */
function normalizeDateString(rawDateStr: string): string | null {
  try {
...
function extractMerchant(
  ...
  for (let i = 0; i < topLines.length; i++) {
    ...
    if (pricePattern.test(rawLine)) continue;
    ...
    let confidence = 0.92;
    if (i > 2) confidence = 0.78;
    if (i > 5) confidence = 0.65;
...
function extractTotalAmount(
  ...
  const candidateAmounts: Array<{ amount: number; line: string; lineIndex: number }> = [];
  ...
  if (candidateAmounts.length > 0) {
    ...
    const maxCandidate = candidateAmounts.reduce((prev, curr) =>
  ...
  if (cleanMerchant) {
    for (const [categoryName, regex] of Object.entries(CATEGORY_KEYWORDS)) {
      if (regex.test(cleanMerchant)) {
        const matchedUserCategory = categories.find(
...
function extractLineItems(lines: string[]): ReceiptLineItem[] {
  ...
  for (const line of lines) {
    ...
    if (
      /total|subtotal|tax|balance|change|cash|visa|mastercard|amex|debit|credit/i.test(
        clean
      )
    ) {
      continue;
...
export function parseReceiptOcr(
  ...
  const categoryField = suggestCategory(
    merchantField.value,
    rawText,
    categories,
  ...
  const overallConfidence = Math.max(
    0,
    Math.min(1, Math.round(Math.min(rawOcrConfidence, fieldsAvg) * 100) / 100)
  );
