...
export function EventCard({
  ...
  if (variant === "block") {
    return (
      <button
        ...
        style={style}
        className={cn(
          "absolute left-0 right-0 z-10 flex flex-col gap-0.5 overflow-hidden rounded border-l-2 border-[#0075de] bg-[#e8f1fb] px-1.5 py-1 text-left text-xs text-[#003a6b] shadow-sm transition-colors hover:bg-[#d7e9fa]",
          className
        )}
      >
  ...
  return (
    <button
      ...
      style={style}
      className={cn(
        "flex w-full items-center gap-1 overflow-hidden rounded px-1.5 py-0.5 text-left text-xs text-[#003a6b] transition-colors hover:bg-[#e8f1fb]",
        className
      )}
    >
      <span className="size-1.5 shrink-0 rounded-full bg-[#0075de]" />
      {!occurrence.isAllDay && (
